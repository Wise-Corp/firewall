...
Specify admin IP to not prevent the admin from accessing the remote host. `admin_ip`
will be used in additional (custom) rules. `admin_ip` must respect `regex("^(?:[0-9]{1,3}\.){3}[0-9]{1,3}\/32$")`.
...
Defines the default chains policies.
...
Whether to drop the rest of packets. `firewall_drop_other_packets` is useful when chains 
policies are equal to `ACCEPT`.
